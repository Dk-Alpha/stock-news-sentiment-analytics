...
async def process_source(source, session, producer, seen_hashes):
    ...
    for entry in feed.entries:
        ...
        # Send to Kafka
        try:
            await producer.send_and_wait(RAW_NEWS_TOPIC, json.dumps(news_event).encode("utf-8"))
        except Exception as e:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            while True:
                ...
                tasks = [process_source(source, session, producer, seen_hashes) for source in current_sources]
